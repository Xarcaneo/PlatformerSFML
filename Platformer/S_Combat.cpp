...
void S_Combat::Update(float l_dT){
	...
	for (auto &entity : m_entities){
		...
		if (!entities->GetComponent<C_Movable>(entity, Component::Movable)) continue;
		Direction dir = entities->GetComponent<C_Movable>(entity, Component::Movable)->GetDirection();
		sf::Vector2f position = entities->GetComponent<C_Body>(entity, Component::Body)->GetPosition();
...
void S_Combat::Notify(const Message& l_message){
	...
	case EntityMessage::Being_Attacked:
		...
		if (attackerDirection == Direction::Left) { KnockbackVelocity.x = -Knockback; }
		else if (attackerDirection == Direction::Right) { KnockbackVelocity.x = Knockback; }
		KnockbackVelocity.y = -Knockback;
		entities->GetComponent<C_Body>(l_message.m_receiver, Component::Body)->SetVelocity(KnockbackVelocity);
		break;
	}
}