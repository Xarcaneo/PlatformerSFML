...
void S_Sound::Update(float l_dT){
	...
	for (auto &entity : m_entities){
		...
		C_Body* c_bod = entities->GetComponent<C_Body>(entity, Component::Body);
		c_pos->SetPosition(c_bod->GetPosition());
...
void S_Sound::Notify(const Message& l_message){
	...
	case EntityMessage::Frame_Change:
		...
		else if (state == EntityState::Jumping) { sound = EntitySound::Jump; }
